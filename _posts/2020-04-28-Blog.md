...
#### 账号注册
   ...
   1. ###### GitHub 注册
      ...
      > **STEP 1.**  新用户前往 [GitHub](https://github.com/join?source=header-home) 页面，进行注册（创建 `用户名username` ，填写 `email` 和设定密码）。
      >
   ...
   3. ###### Disqus 注册
      ...
      > **STEP 1.**  打开 [Disqus](https://disqus.com/) 网址页面，首先注册一个 disqus 账号。进入设置 `Settings` ，修改信息
      >
   ...
   4. ###### 域名注册
      ...
      > **STEP 1.**  网上有很多推荐渠道，这里给一个阿里云[域名注册](https://wanwang.aliyun.com/domain/com/?spm=5176.10695662.1158081.1.598542349LxUeh)链接
      >
      ...
      >
      > 1. 在[域名列表](https://dc.console.aliyun.com/next/index#/domain/list/all-domain)里，找到你购买的域名，进行 `解析` 。
      >
      ...
      >
      >    也可以通过 [IP 查询网站](http://ip.chinaz.com/) ，得到相应 IP 地址。例如：
      >
   ...
   5. ###### 网站统计
      ...
      > 这里是两大统计网站  [Baidu Analytics](https://tongji.baidu.com/web/welcome/login?castk=LTE%3D) 和 [Google Analytics](https://analytics.google.com/analytics/) ，到各个网站按照流程注册得到相对应的 `track_id` ，在《博客改造》的 _config.yml 中填入。
      >
...
#### git配置 + SSH
   ...
   > **STEP 1.**  前往 [git](https://git-scm.com/downloads) 页面下载对应版本，安装路径自己选择。
   >
...
#### Jekyll配置
   ...
   ***这一步是用来熟悉 jekyll 搭建静态博客，方便以后在本地修改文件预览效果，提高效率。***
   ...
   > **STEP 2.**  下载 [RubyGems](https://rubygems.org/pages/download) ，解压缩 .zip 文件后，运行 setup.rb ，输入 gem -v ，查看是否安装成功。
   >
...
#### 博客模板下载
   ...
   > **STEP 1.**  采用文章末尾链接中的博客模板，或是使用我的改进版，也可以通过其他渠道下载。但不管是通过 [jekyll theme 1](https://jekyllthemes.io/) , [jekyll theme 2](http://jekyllthemes.org/) , [知乎](https://www.zhihu.com/) 还是别的渠道获取模板，都建议进入github页面下载。
   >
...
#### 博客改造
   ...
   ***此处针对 HUX 的模板进行改造，其他模板流程大体相同，细节处结合自己的模板。***
   ...
   > **PART 3.**  _layouts
   >
   > 1. 针对 keynote.html 和 post.html 进行了少量的修改，插入了 Disqus（发评论用）。如若不会，可以直接 copy 我的代码，推荐查看[0xC00005的文章](https://www.jianshu.com/p/7e4453421b8f)，因为我后续可能还会有改动。
   >
